...
%% Density 
plot_den=zeros(ftd,ffd);
Tv=zeros(ftd,ffd);
dzz=(Radii-maxheight)/hd;
Tv(:,:)=K*plotT(:,:)./mmass.*mole;
Q=10^14;
den_atm_2=zeros(hd,ftd,ffd);
for ht=1:hd
    Area=4*pi*(Radii+ht*hd)^2/(ftd*ffd);
    plot_den(:,:)=den_atm(ht,:,:);
    plot_den(:,:)=plot_den(:,:).*Q./n./Tv(:,:);
    den_atm_2(ht,:,:)=plot_den(:,:);
    disp(ht)
end
%%
...
plot_den(:,:)=den_atm_2(:,:,ffd/2)
aa=zeros(hd,ftd);
for ht=1:hd
    aa(ht,:)=plot_den(hd-ht+1,:);
end
imagesc(aa);
hold on;
colorbar
...
str=num2str(RR);
str2=num2str(RR2);
set(gca,'yticklabel',{str,str2,'horizon'});
xlabel('latitude');
ylabel('distance [km]');
title('Local Time = 12:00, density [molecules/m^3]');
%%
figure(3)
